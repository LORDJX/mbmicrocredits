...
# Modelo para la información del cliente que se anidará en la respuesta del préstamo
...
class LoanResponse(LoanBase):
    ...
    clients: Optional[ClientInfo] = None # Campo para anidar la info del cliente
...
async def get_all_loans(
    search: Optional[str] = Query(None, description="Buscar por código de préstamo, código de cliente, o nombre/apellido del cliente"),
    ...
    La búsqueda funciona en el código del préstamo, y en el código, nombre o apellido del cliente asociado.
    """
    try:
        # Seleccionamos los datos del préstamo y la información relevante del cliente asociado
        query = db_client.from_("loans").select("*, clients(client_code, first_name, last_name)")

        if search:
            # Usamos 'or' para buscar en múltiples campos, incluyendo los de la tabla anidada 'clients'
            query = query.or_(
                f"loan_code.ilike.%{search}%,clients.client_code.ilike.%{search}%,clients.first_name.ilike.%{search}%,clients.last_name.ilike.%{search}%"
            )
...
async def get_loan_by_id(loan_id: UUID, db_client: Client = Depends(get_db_client)):
    """
    Recupera un préstamo específico por su ID, incluyendo información del cliente.
    """
    try:
        response = db_client.from_("loans").select("*, clients(client_code, first_name, last_name)").eq("id", str(loan_id)).single().execute()
...
async def create_loan(loan_data: LoanCreate, db_client: Client = Depends(get_db_client), is_admin: bool = Depends(is_request_admin)):
    ...
    try:
        ...
        # Después de crear, obtenemos el préstamo con la info del cliente para devolverlo
        new_loan_id = response.data[0]['id']
        return await get_loan_by_id(new_loan_id, db_client)
...
async def update_loan(loan_id: UUID, loan_data: LoanUpdate, db_client: Client = Depends(get_db_client), is_admin: bool = Depends(is_request_admin)):
    ...
    try:
        ...
        # Después de actualizar, obtenemos el préstamo con la info del cliente para devolverlo
        return await get_loan_by_id(loan_id, db_client)
