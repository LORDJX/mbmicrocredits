...
export async function GET(request: NextRequest) {
  try {
    ...
    if (installmentsError) {
      ...
      const { data: basicInstallments, error: basicError } = await supabase
        .from("installments")
        .select(`
          id,
          loan_id,
      ...
      if (basicError) {
        console.error("[v0] Error fetching basic installments:", basicError)
        return NextResponse.json({ error: "Error fetching installments" }, { status: 500 })
      }

      const enrichedInstallments = []
      for (const installment of basicInstallments || []) {
        ...
        if (loan) {
          enrichedInstallments.push({
            ...
            calculated_status: installment.status,
          })
        }
      }

      console.log("[v0] Total installments found:", enrichedInstallments.length)
    ...
    const processedInstallments = allInstallments.map((installment) => {
      ...
      if (isPaid) {
        status = "paid"
      } else if (installment.calculated_status === "con_mora") {
        status = "overdue"
    ...
    const statusCounts = {
      ...
      pagadas_con_mora: paidInstallments.filter((i) => i.calculated_status === "con_mora").length,
    }
