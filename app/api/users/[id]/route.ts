...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    console.error(`Error al proxy GET /api/users/${id}:`, error)
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    console.error(`Error al proxy PATCH /api/users/${id}:`, error)
    return NextResponse.json({ detail: "Error interno del servidor" }, { status: 500 })
  }
}
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    console.error(`Error al proxy DELETE /api/users/${id}:`, error)
    return NextResponse.json({ detail: "Error interno del servidor" }, { status: 500 })
  }
}
