  ...
  try {
    ...
    if (error) {
      return NextResponse.json(
        { success: false, detail: "Error al obtener cliente", error: { message: error.message, code: error.code } },
        { status: status || 500 },
      )
    }
    ...
    return NextResponse.json({ success: false, detail: String(err) }, { status: 500 })
  }
}
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { success: false, detail: "Error al actualizar cliente", error: { message: error.message, code: error.code } },
        { status: status || 500 },
      )
    }
  ...
  try {
    ...
    if (error) {
      return NextResponse.json(
        {
          success: false,
          detail: "Error al eliminar (soft) cliente",
    ...
    return NextResponse.json(
      { success: true, message: "Cliente eliminado (soft delete) correctamente.", data },
      { status: 200 },
    )
  } catch (err: any) {
