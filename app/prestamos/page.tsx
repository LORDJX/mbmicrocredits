...
import { LoanStatusSelector } from "@/components/loan-status-selector"
...
interface Loan {
  ...
  installments: number
  loan_type: string
  interest_rate: string // Tasa de interés (valor en porcentaje, ej: "90")
  start_date: string
...
async function getLoansWithBalance() {
  try {
    ...
    // Obtener saldo de cada préstamo
...
export default async function PrestamosPage() {
  ...
  const activeLoansCount = loans.filter((loan) => loan.status === "active" || loan.status === "Activo").length
  ...
  const completedLoansCount = loans.filter((loan) => loan.status === "completed" || loan.status === "Completado").length
  ...
  const getStatusBadge = (status: string) => {
    switch (status?.toLowerCase()) {
      ...
      case "pendiente":
        return (
          <Badge variant="outline" className="border-orange-500 text-orange-800">
                        Pendiente          {" "}
          </Badge>
        )
      default:
              ...
              <TableBody>
                ...
                {loans.map((loan) => (
                  <TableRow key={loan.id} className="border-border">
                    ...
                                       {" "}
                    <TableCell>
                      <LoanStatusSelector loanId={loan.id} currentStatus={loan.status} />
                    </TableCell>
                                       {" "}
                    <TableCell>
                                           {" "}
                      <div className="flex gap-2">
                        ...
                        <Button variant="outline" size="icon" className="border-border bg-transparent">
                                                    <Eye className="h-4 w-4" /> {/* Botón Ver */}                       {" "}
                        </Button>
                                               {" "}
