...
interface Loan {
  ...
  balance: number
  has_pending_installments: boolean // ✅ NUEVO
}
...
async function getLoansWithBalance() {
  try {
    ...
    const { data: loans, error } = await supabase
      ...
      `)
      .is("deleted_at", null) // ✅ Solo préstamos activos
    ...
    // ✅ CORRECCIÓN: Obtener saldo Y verificar si tiene cuotas pendientes
    const loansWithBalance = await Promise.all(
      (loans || []).map(async (loan) => {
        ...
        // ✅ NUEVO: Verificar si tiene cuotas pendientes
...
export default async function PrestamosPage() {
  ...
  // ✅ CORRECCIÓN: Cálculo correcto de préstamos activos
  ...
  const getStatusBadge = (loan: Loan) => {
    // ✅ CORRECCIÓN: Badge basado en cuotas pendientes, no en status
