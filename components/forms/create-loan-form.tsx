...
export function CreateLoanForm({ onSuccess, onCancel, preselectedClientId, initialData }: CreateLoanFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    installment_amount: initialData?.installment_amount?.toString() || "",
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || `Error al ${isEditMode ? "actualizar" : "crear"} préstamo`)
      }

      toast({
        title: isEditMode ? "Préstamo actualizado" : "Préstamo creado",
        description: `Préstamo ${data.loan.loan_code} ${isEditMode ? "actualizado" : "creado"} exitosamente`,
      })
      ...
      toast({
        title: "Error",
        description:
          error instanceof Error ? error.message : `Error al ${isEditMode ? "actualizar" : "crear"} préstamo`,
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
