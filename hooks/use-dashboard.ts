  ...
  useEffect(() => {
    const fetchDashboardStats = async () => {
      try {
        ...
        const activeLoans = Array.isArray(loans) ? loans.filter((loan: any) => loan.status === "activo").length : 0

        const completedLoans = Array.isArray(loans)
          ? loans.filter((loan: any) => loan.status === "completado").length
          : 0

        const totalRevenue = Array.isArray(transactions)
          ? transactions.reduce((sum: number, transaction: any) => {
              return sum + (Number.parseFloat(transaction.amount) || 0)
            }, 0)
          : 0
